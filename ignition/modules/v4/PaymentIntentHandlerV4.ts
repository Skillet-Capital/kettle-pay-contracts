...
const PaymentIntentHandlerV4 = buildModule("PaymentIntentHandlerV4", (m) => {
  const usdc = m.getParameter("usdc");
  const { cctpMintHookWrapper } = m.useModule(CCTPMintHookWrapper);
  const cctpPaymentIntentReceiver = m.contract("PaymentIntentHandlerV4", [
    usdc,
    cctpMintHookWrapper,
  ]);
...
export default PaymentIntentHandlerV4;
