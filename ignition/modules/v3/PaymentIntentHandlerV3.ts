...
const PaymentIntentHandlerV3 = buildModule("PaymentIntentHandlerV3", (m) => {
  const usdc = m.getParameter("usdc");
  const { cctpMintHookWrapper } = m.useModule(CCTPMintHookWrapper);
  const cctpPaymentIntentReceiver = m.contract("PaymentIntentHandlerV3", [
    usdc,
    cctpMintHookWrapper,
  ]);
...
export default PaymentIntentHandlerV3;
