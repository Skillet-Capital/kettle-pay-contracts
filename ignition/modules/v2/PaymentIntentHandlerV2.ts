...
const PaymentIntentHandlerV2 = buildModule("PaymentIntentHandlerV2", (m) => {
  const usdc = m.getParameter("usdc");
  const { cctpMintHookWrapper } = m.useModule(CCTPMintHookWrapper);
  const cctpPaymentIntentReceiver = m.contract("PaymentIntentHandlerV2", [
    usdc,
    cctpMintHookWrapper,
  ]);
...
export default PaymentIntentHandlerV2;
